...
public class BookshopRest {
    ...
    private static final String AUTH_TOKEN = "42";


    private boolean isAcceptedFormat(String acceptHeader) {
        return MediaType.APPLICATION_JSON.equals(acceptHeader) || MediaType.APPLICATION_XML.equals(acceptHeader);
    }
    ...
    public Response getBooks(@QueryParam("author") String author, @QueryParam("title") String title, @HeaderParam(HttpHeaders.ACCEPT) String acceptHeader) {
        if (!isAcceptedFormat(acceptHeader)) {
    ...
    @Path("books/{id}")
    @Produces({ MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML })
    public Response getBook(@PathParam("id") Integer id, @HeaderParam(HttpHeaders.ACCEPT) String acceptHeader) {
        if (!isAcceptedFormat(acceptHeader)) {
            return Response.status(Status.NOT_ACCEPTABLE).build();
        }
    ...
    @DELETE
    @Path("books/{id}")
    public Response deleteBook(@PathParam("id") Integer id, @HeaderParam(HttpHeaders.AUTHORIZATION) String auth) {
